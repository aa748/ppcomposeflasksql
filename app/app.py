...
def record_view(player_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tblPlayersImport WHERE player_id=%s', player_id)
    result = cursor.fetchall()
...
def form_update_post(player_id):
    ...
    sql_update_query = """UPDATE tblPlayersImport t SET t.fldName = %s, t.fldTeam = %s, t.fldPosition = %s, t.fldHeight_inches = 
    %s, t.fldWeight_lbs = %s, t.fldAge = %s, t.fldPopulation = %s WHERE t.player_id = %s """
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO tblPlayersImport (fldName,fldTeam,flPosition,fldHeight_inches,fldWeight_lbs) VALUES (%s, %s,%s, %s,%s) """
...
def form_delete_post(player_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM tblPlayersImport WHERE player_id = %s """
